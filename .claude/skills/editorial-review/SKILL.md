...
6. **Lead/conclusion + format compliance** — hook anchor to thesis, frame closure, `# Sources` 5-category enum check, "First, et al." format, em-dash count, `[xxxx]` format check, banned-words grep (covers the deterministic-gate absorption), and the 6G over-hedge guard (verdict confidence proportionate to evidence — the mirror of Pass 3/4's overreach guard, held against thesis-spine's declared closing posture). See `references/pass-6-lead-conclusion-format.md`.

...
3. **Claim adequacy + fact verification + paraphrase mutation** — every `[XXXX]` cite verified against `invention-summary.md` Quotable spans verbatim; external claims *flagged offline* against `fact-check-log.md` (live web re-verification is reassigned to the publication-threshold `prepublish-verify` stage — see `references/pass-3-fact-paraphrase.md` §3B); paraphrase variations classified (intentional restatement / accidental drift / substantive change). See `references/pass-3-fact-paraphrase.md`.

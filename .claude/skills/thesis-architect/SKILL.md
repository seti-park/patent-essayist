...
3. **Thesis candidate generation** — 2-4 candidates, single-spine default. Each candidate carries draft 4-axis grounding **plus two mandatory one-sentence fields: the 통념 it overturns and the 전복 (how the patent evidence contradicts it)** — the seed of the thesis card. Write `thesis-candidates.md` capturing each candidate's frame + 통념/전복 + 4-axis status + rejection reason (for rejected ones). See `references/thesis-candidate-presentation.md`.
4. **4-axis grounding lock** — for each candidate, fill all 4 axes (claims / problem / effect / baseline-difference). Any missing axis disqualifies the candidate. See `references/4-axis-grounding.md`.
   - **Step 4.5 — card gate**: per candidate, fill the `### Card-gate checklist` in `thesis-candidates.md`: 4-axis 4/4, **consensus citations ≥1 (hard; ≥2 clean)** — external sources from Step 2 context research proving someone actually holds the 통념 (log in `search-log.md`, seed into `fact-check-log.md`), Q7 mappability, arc-sustainability. A hard-item failure disqualifies the candidate before the tournament. Mirrors `_shared/scripts/check_thesis_card.py` (TCARD-*), which the orchestrator runs on the locked spine.
...
6. **Adversarial defense** — surface the strongest objection per surviving candidate, draft mitigation. Context research's **layer-confusion findings** are priority inputs for Category 1 objections. **Then run the pre-selection red-team (3 attack vectors: strawman consensus / triviality reduction / scope overreach)** on every card-gate survivor; record survived/killed per candidate in `thesis-candidates.md` `### Red-team survival` (one revise-and-retry per candidate). See `references/adversarial-defense.md`.
7. **Selection** — survivors 2+: run the **pairwise tournament** (single criterion: "which one more decisively overturns a better-evidenced 통념?" — relative judgment only, NO absolute scores) and record matches + winner in `thesis-candidates.md` `## Tournament`, plus a `## Selection rationale` (this is what the orchestrator surfaces as the thesis card at the `--thesis-gate` checkpoint). Survivor 1: skip the tournament, record why. **Survivors 0: return to Step 3 and regenerate candidates (P1 inner loop, max 2 rounds); if exhausted, surface the least-bad candidate + failure reasons at the checkpoint.** Single-spine default; multi-spine requires explicit override per `references/single-spine-default.md`.
8. **Spine lock** — write `thesis-spine.md` with locked candidate's 4-axis anchors, **a `## Consensus evidence` section (mandatory: the 통념 one-liner + its external citations + the 전복 one-liner — TCARD-003 hard-fails a spine with zero citations)**, Q7 pattern, adversarial defense, spine→section trace, and an **`## Arc budget`**: declare THIS essay's arc as roles with a % share each (summing to ~100), marking any single-occurrence role `(once)`. This is the length+structure DECISION made once, here in Phase 1 — Phase 2 derives section `word_target`s from it and `gate_arc` checks conformance. Declare the arc the essay actually has (recommended roles: lead, context, development, turn/reversal, implication, closing); do NOT force a fixed shape. When `audience=investor`, also declare a `reader_stake` field (what the investor reader decides or gets). For `deep` the field may be omitted.
...
- `handoff/01-design/thesis-spine.md` exists; selected candidate has all 4 axes anchored, Q7 hook pattern declared, and `## Consensus evidence` carries ≥1 external citation (`check_thesis_card.py` returns PASS).
...
- Step 6 red-team killing all candidates forces return to Step 3 (max 2 P1 inner-loop rounds).

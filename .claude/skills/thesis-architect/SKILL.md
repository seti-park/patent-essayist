...
8. **Spine lock** — write `thesis-spine.md` with locked candidate's 4-axis anchors, Q7 pattern, adversarial defense, **closing posture**, spine→section trace. Closing posture defaults by edition: verdict / investor / analysis → `firm-closing` (lead the verdict with the call, caveats stated once, one anti-hype guard — Pass 6 6G holds it); mechanism / explainer → `measured`. Declare it here, at design time: the Compose↔Edit loop's Pass 3/4 only push claims down, so an undeclared posture converges over-defensive (the run-045 lesson: pinning it made the loop pass in one round).
